...
def locate_templates(img, templates, start, stop, threshold):
    ...
    for i, template in enumerate(templates):
        if i < len(locations) and len(locations[i]) > 0:  # Check if the index exists and is not empty
            w, h = template.shape[::-1]
            w *= scale
            h *= scale
            img_locations.append([BoundingBox(pt[0], pt[1], w, h) for pt in zip(*locations[i][::-1])])
        else:
            print(f"No matches found for template {i} with threshold: {threshold}")
            img_locations.append([])  # Append an empty list or handle as needed
